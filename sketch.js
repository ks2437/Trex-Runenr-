...
function setup() {
  createCanvas(windowWidth, windowHeight);
    ib = createSprite(width/2, height/2, windowWidth, windowHeight);
  ib.visible=false
  trex = createSprite(50, height/2, 20, 20);
  ground_2 = createSprite(width/2, height, width, 20);
  ground_2.visible = false
  c1 = createSprite(width-400, 30, 10, 10);
  c1.velocityX = -5;
  c1.addImage(ci);
  c2 = createSprite(width-100, 45, 10, 10);
  c2.velocityX = -5;
  c2.addImage(ci);
...
function cc() {
  if (frameCount % 60 == 0) {
    var clouds = createSprite(width, Math.round(random(10, height/2-30)), 10, 10);
    clouds.velocityX = speed;
    clouds.addImage(ci);
    clouds.depth = trex.depth;
    trex.depth = trex.depth + 1;
    clouds.lifetime = width / -speed + 40;
    cg.add(clouds);
  }
}
...
function ct() {
  if (frameCount % 100 == 0) {
    var cactus = createSprite(width, height-30, 10, 10);
    cactus.velocityX = speed;
    cactus.lifetime = width / -speed + 40;
    var n;
    if(SCORE<100)
...
function avr() {
  if (frameCount % 567 == 0) {
    var avr = createSprite(width, Math.round(random(80, height/2+20))); 
    avr.lifetime = width1 / -speed + 40;
...
function draw() {
  ...
  if (gs == 0) {
   ...
   ct();
  avr();
    if ((keyDown("space")||touches.length>0) && trex.y > height-33.5) {
      trex.velocityY = -10;
      jump.play();
    }
    trex.velocityY += .5;
